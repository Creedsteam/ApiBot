...
function JSON:onDecodeOfHTMLError(message, text, x, etc)
  if text then
    if location then
      message = string.format("%s at char %d of: %s", message, location, text)
    else
...
function JSON:onDecodeOfNilError(message, x1, x2, etc)
  if text then
    if location then
      message = string.format("%s at char %d of: %s", message, location, text)
    else
...
local function configure(token)
  print("you are using the ApiBot source by @Mouamle")
  if (token == "") then
    token = nil
  end
...
local function sendKB(chat_id, text, reply_markup, reply_to_message_id)
  ...
  request_body.chat_id = chat_id
  request_body.text = tostring(text)
  request_body.parse_mode = ""
  request_body.disable_web_page_preview = tostring(true)
  request_body.disable_notification = tostring(false)
  request_body.reply_to_message_id = tonumber(reply_to_message_id)
  request_body.reply_markup = reply_markup or ""
